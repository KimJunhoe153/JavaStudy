...
insert into 부서 values ('A5', '마케팅부'); -- 컬럼 순서대로 작성해야함
insert into 제품 values (91, '연어피클소스', null, 5000, 40);

-- insert into 테이블명 (컬럼1, 컬럼2, ..)
-- values (값1, 값2, ..);
-- 이 경우, values는 위의 컴럼 순서대로만 써주면 됨
...
-- update
-- update 테이블명 set 컬럼1 = 값1, 컬럼2 = 답2, ..
-- where 조건
update 사윈
set 이름 = '김레몬'
where 사원번호 = 'E20'; -- where조건이 없으면 모든 열이 변경됨
...
-- 가장 최근 입사자로 순서로 정렬하고 위에서 3개 row를 삭제
delete from 사원
order by 입사일 desc
limit 3;

-- insert on duplicate key update
-- 레코드 (= 열, row, 튜플)가 ㅇ벗으면 새롭게 추가하고, 있으면 변경
insert into 제품 (제품번호, 제품명, 단가, 재고)
